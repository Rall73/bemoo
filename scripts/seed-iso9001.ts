/**
 * Seed: Template de Auditoria ISO 9001:2015
 * Executar: npx tsx scripts/seed-iso9001.ts
 *
 * Pré-requisito: banco atualizado com os campos reference, reference_source,
 * allow_na (checklist_item_fields) e is_template, template_source (checklists).
 */
...
const TEMPLATE_DESC   = "Checklist completo de auditoria interna baseado nos requisitos da norma ISO 9001:2015 (Sistema de Gestão da Qualidade). Inclui 71 perguntas em 13 grupos temáticos com referências normativas por cláusula."
...
const GRUPOS: GroupDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { label: "A Alta Direção demonstra comprometimento com o foco no cliente, garantindo que os requisitos dos clientes são determinados e atendidos?", clausula: "5.1.2", description: "Evidência: mecanismos de captura de requisitos e monitoramento da satisfação do cliente." },
  ...
  {
    title: "6. Objetivos da Qualidade e Planejamento (Cláusula 6.2–6.3)",
    fields: [
      ...
      { label: "Existe planejamento sobre como atingir os objetivos da qualidade (o quê, recursos, responsável, prazo, forma de avaliação)?", clausula: "6.2.2", description: "Evidência: plano de metas com os 5 elementos requeridos pela norma." },
      { label: "Os objetivos da qualidade são consistentes com a Política da Qualidade e consideram os requisitos aplicáveis?", clausula: "6.2.1", description: "Evidência: rastreabilidade entre política, objetivos e indicadores." },
      { label: "Mudanças no SGQ são realizadas de forma planejada, considerando propósito, integridade do SGQ, recursos disponíveis e responsabilidades?", clausula: "6.3", description: "Evidência: registros de análise de impacto de mudanças no SGQ." },
    ],
  },
  {
    ...
    fields: [
      ...
      { label: "O ambiente para a operação dos processos (físico, social, psicológico) é determinado, provido e mantido conforme necessário?", clausula: "7.1.4", description: "Evidência: registros de condições ambientais monitoradas, políticas de bem-estar." },
      ...
      { label: "O conhecimento organizacional necessário para a operação dos processos e conformidade de produtos/serviços é determinado, mantido e disponibilizado?", clausula: "7.1.6", description: "Evidência: base de conhecimento, procedimentos, lições aprendidas documentadas." },
    ],
  },
  {
    ...
    fields: [
      ...
      { label: "Ações para adquirir a competência necessária são tomadas e sua eficácia é avaliada?", clausula: "7.2", description: "Evidência: plano de treinamento com avaliação de eficácia (pré e pós-teste, observação)." },
      { label: "As pessoas que trabalham sob controle da organização estão conscientes da política, objetivos, sua contribuição para a eficácia do SGQ e as implicações de não estar em conformidade?", clausula: "7.3", description: "Evidência: registros de treinamento de conscientização, comunicados internos." },
      { label: "A organização determina as comunicações internas e externas pertinentes ao SGQ (o quê, quando, com quem, como e quem comunica)?", clausula: "7.4", description: "Evidência: plano ou matriz de comunicação documentada." },
    ],
  },
  {
    ...
    fields: [
      ...
      { label: "A criação e atualização de informação documentada incluem identificação, formato adequado e análise crítica e aprovação?", clausula: "7.5.2", description: "Evidência: procedimento de controle de documentos com cabeçalho padrão (ID, revisão, aprovação)." },
      { label: "A informação documentada é controlada quanto à disponibilidade, proteção e acesso adequado (leitura/escrita por quem autorizado)?", clausula: "7.5.3", description: "Evidência: controles de acesso no sistema de gestão documental." },
      ...
      { label: "A informação documentada retida como evidência de conformidade é protegida contra modificações não intencionais?", clausula: "7.5.3", description: "Evidência: registros armazenados em formato que impede alteração (PDF assinado, sistema com log de alterações)." },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { label: "As saídas dos processos são preservadas (identificação, manuseio, armazenamento, transporte, proteção) para assegurar a conformidade?", clausula: "8.5.4", description: "Evidência: procedimentos de armazenamento, condições ambientais controladas, embalagem adequada." },
      { label: "As atividades de liberação de produtos e serviços são implementadas conforme planejado e evidenciadas por informação documentada?", clausula: "8.6", description: "Evidência: registros de inspeção final, laudos de aprovação, assinatura do responsável pela liberação." },
  ...
  },
  {
    ...
    fields: [
      ...
      { label: "A análise crítica pela direção aborda todas as entradas requeridas pela norma (resultados de auditorias, desempenho de processos, satisfação de clientes, etc.)?", clausula: "9.3.2", description: "Evidência: pauta e ata de análise crítica cobrindo todos os itens da cláusula 9.3.2." },
  ...
  },
  {
    ...
    fields: [
      ...
      { label: "Não conformidades e ações corretivas são retidas como informação documentada?", clausula: "10.2.2", description: "Evidência: sistema de registro de NCs (software, planilha ou formulário) com histórico." },
...
async function main() {
  console.log("🔍 Verificando template ISO 9001:2015...")
  ...
  if (existing) {
    console.log(`⚠️  Template já existe (id=${existing.id}). Abortando para evitar duplicata.`)
    console.log("   Para recriar, delete o template existente primeiro.")
    return
  }

  // Encontra o primeiro platform admin para usar como createdBy
  const admin = await prisma.user.findFirst({
    where: { platformAdmin: true, deletedAt: null },
    orderBy: { id: "asc" },
  })
  if (!admin) {
    console.error("❌ Nenhum platform admin encontrado no banco. Crie um usuário com platform_admin=1 primeiro.")
    process.exit(1)
  }
  console.log(`👤 Usando usuário "${admin.name}" (id=${admin.id}) como criador.`)

  // Conta total de campos
  const totalFields = GRUPOS.reduce((s, g) => s + g.fields.length, 0)
  console.log(`📋 Criando template com ${GRUPOS.length} seções e ${totalFields} campos...`)

  // Cria o template em transação
  await prisma.$transaction(async (tx) => {
    const template = await tx.checklist.create({
      data: {
        companyId:      null,
        name:           TEMPLATE_NAME,
      ...
      for (const campo of grupo.fields) {
        await tx.checklistItemField.create({
          data: {
            ...
            type:            "SIM_NAO",   // Conforme / Não Conforme
            required:        true,
            requirePhoto:    false,
            allowNa:         true,         // N/A é essencial em auditorias normativas
    ...
    console.log(`   Acesse: /plataforma/templates para gerenciar`)
    console.log(`   As empresas podem importar em: /checklists`)
  })
}
