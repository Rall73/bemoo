...
export default async function ResultadoPage({
  ...
  const itemNoteMap = new Map(execution.itemNotes.map((n) => [n.itemId, n]))
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      ...
      <div className="bg-white border border-gray-200 rounded-round p-6 text-center space-y-3">
        ...
        <div className="flex items-center justify-center gap-6 pt-1 text-sm text-gray-600 flex-wrap">
          ...
          {conformes > 0 && <span className="text-success font-medium">{conformes} ✓</span>}
          {naoConf > 0  && <span className="text-error font-medium">{naoConf} ✗</span>}
          {naCount > 0  && <span className="text-gray-400 font-medium">{naCount} N/A</span>}
        </div>
      </div>
      ...
      {execution.checklist.items.map((item, itemIdx) => {
        const itemNote = itemNoteMap.get(item.id)
        const hasNote  = !!(itemNote?.photoUrl || itemNote?.annotation?.trim() || itemNote?.transcription?.trim())
        ...
        return (
          <div key={item.id} className="bg-white border border-gray-200 rounded-round overflow-hidden">
            ...
            {/* Notas do item (foto, anotação, transcrição) */}
            {hasNote && (
              <div className="px-4 py-3 bg-gray-50/70 border-t border-gray-100 space-y-2.5">
                {itemNote?.photoUrl && (
                  <a href={itemNote.photoUrl} target="_blank" rel="noopener noreferrer">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={itemNote.photoUrl}
                      alt={`Foto — ${item.label}`}
                ...
                {itemNote?.annotation?.trim() && (
                  <div className="flex items-start gap-1.5">
                    <MessageSquare size={12} className="text-gray-400 mt-0.5 flex-shrink-0" strokeWidth={2} />
                    <p className="text-xs text-gray-600 leading-snug">{itemNote.annotation}</p>
                  </div>
                )}

                {itemNote?.transcription?.trim() && (
                  <div className="flex items-start gap-1.5 bg-primary-50/30 border border-primary-100 rounded-soft px-2 py-1.5">
                    ...
                    <p className="text-xs text-gray-700 italic leading-snug">{itemNote.transcription}</p>
                  </div>
                )}
              </div>
            )}
